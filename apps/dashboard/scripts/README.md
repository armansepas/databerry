...
These scripts are located in `apps/dashboard/scripts/` within the workspace so they can properly access the `@chaindesk/prisma` package and other workspace dependencies.
...
npx tsx scripts/setup-premium-test-user.ts test@example.com

# Create with specific plan
npx tsx scripts/setup-premium-test-user.ts test@example.com level_2

# Create with default email and plan
npx tsx scripts/setup-premium-test-user.ts
```
...
npx tsx scripts/modify-user-subscription.ts user@example.com level_0

# Set specific status
npx tsx scripts/modify-user-subscription.ts user@example.com level_2 active
npx tsx scripts/modify-user-subscription.ts user@example.com level_1 canceled
```
...
```bash
npx tsx scripts/view-user-details.ts user@example.com
```
...
```bash
npx tsx scripts/reset-user-usage.ts user@example.com
```
